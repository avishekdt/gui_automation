...
public class RemoteDeployToWin extends TestSuiteBase{
	...
	@Test
	public void test_RemoteDeployToWin() {
		...
		try {
			...
			if (currentOsName.contains("windows")) {
				...
				/*appendHtmlComment("Linux to Windows deployment is not supported.");
			...
			if(!killRemoteHpsum(remoteHostDesc_w1, remoteHostIp_w1, remoteHostUserName1_w1, remoteHostPassword1_w1)) {
				printError("Failed to kill hpsum service in RemoteDeployToWin");
			}
			...
			// 5. Deploy Node.
			//if(!Nodes.deployNode(remoteHostIp_w1, remoteHostType_w1)) {
			//throw new TestException("Test Failed");
			//}
			...
			if(!Nodes.deployNodeAdvMode(remoteHostIp_w1, remoteHostType_w1)) {
				throw new TestException("Test Failed");
			}
