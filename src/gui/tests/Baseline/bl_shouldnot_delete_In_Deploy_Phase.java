...
public class bl_shouldnot_delete_In_Deploy_Phase extends TestSuiteBase {	
	...
	public void test_bl_shouldnot_delete_In_Deploy_Phase() {
		...
		try {
		  ...
		  //3. Adding a node without baseline.			
			...
			if(Nodes.without_baseline || Nodes.wait_deploy){
				Nodes.without_baseline = false; Nodes.wait_deploy = false;
			}
			if (skip) {
				test_result = "SKIP";
			}
