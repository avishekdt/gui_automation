...
public class bl_shouldnot_delete_In_Inventory_Phase extends TestSuiteBase {	
	...
	@Test
	public void test_bl_shouldnot_delete_In_Deploy_Phase() {
		...
		try {
		  ...
		  //3. Adding a node without baseline.			
			...
			//wait_inv variable is setting to true so that, no need to wait for inventory to complete.
			...
			Nodes.wait_inv=false;
			
			// Delete additional baseline after the installation is done.
			if(!BaselineLibrary.deleteAbl(sppLocation_toSelect)){
				printLogs("Baseline is not deleted as delete button is not exists");
				test_result = "PASS";
				//throw new TestException("Test Failed");
			}
			...
			if(Nodes.without_baseline || Nodes.wait_inv){
				Nodes.without_baseline = false; Nodes.wait_inv = false;
			}
			if (skip) {
				test_result = "SKIP";
			}
