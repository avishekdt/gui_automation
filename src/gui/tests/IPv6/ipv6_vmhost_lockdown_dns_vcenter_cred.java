 ...
 * Author: Praveen AC
 * Date: 05/18/2016
 * Test case Description: Deploy on a IPv6 VM host node (unicast/global) which is in lock down mode. 
 * 							DNS name of  IPv6 address of Vcenter should be specified while adding the node.
 * Prerequisites: Configure localhost and remote server node with global IPV6 IP address. Vcenter is added 
 * 					to domain controller.(Optional: Disable IPv4 address)
 * Note: HP SUM needs DNS to be configured properly for the IPV6 ESXi target so that node inventory and deployment is successful.
 * Steps:
 * 1. Launch HPSUM.
 ...
 * Variables to update: remoteHostIpv6g_vm1,vcenterIP_vm1
 */

public class ipv6_vmhost_lockdown_dns_vcenter_cred extends TestSuiteBase{
	...
	@Test
	public void test_ipv6_vmhost_lockdown_dns_vcenter_cred() {
		...
		try {
			...
			// Setup:
			//
			String remoteHostIp_vm1 		= CONFIG.getProperty("remoteHostIpv6g_vm1");
			...
			String vcenterIP_vm1 			= CONFIG.getProperty("vcenterdnsName_vm1");
			String vcentertUserName1_vm1 	= CONFIG.getProperty("vcenterdnsUserName1_vm1");
			String vcenterPassword1_vm1 	= CONFIG.getProperty("vcenterdnsPassword1_vm1");
			...
			printLogs("vCenter IP: " + vcenterIP_vm1);
			printLogs("UserName: " + vcentertUserName1_vm1 + " Password: " + vcenterPassword1_vm1);
			// 1. Do test setup
			...
			// 3. Add Node
			if(!Nodes.addNodeVm(remoteHostIp_vm1, remoteHostDesc_vm1, remoteHostUserName1_vm1, 
					remoteHostPassword1_vm1, true, vcenterIP_vm1, vcentertUserName1_vm1, vcenterPassword1_vm1)) {
				throw new TestException("Test Failed");
			}
