 ...
 * Author: Praveen AC
 * Date: 05/18/2016
 * Test case Description: Deploy on a VM host node which was added with its IPv6 address (global/unicast).
 * Prerequisites: Configure localhost and remote server node with global IPV6 IP address. (Optional: Disable IPv4 address)
 ...
 * Variables to update: remoteHostIpv6g_vm1
 */

public class ipv6_vmhost_deploy extends TestSuiteBase{
	...
	@Test
	public void test_ipv6_vmhost_deploy() {
		...
		try {
			...
			if(!Nodes.addNodeVm(remoteHostIp_vm1, remoteHostDesc_vm1,remoteHostUserName1_vm1, 
					remoteHostPassword1_vm1, false, vcenterIP_vm1, vcentertUserName1_vm1, vcenterPassword1_vm1)) {
				throw new TestException("Test Failed");
			}
