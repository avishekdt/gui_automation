...
// Assertion: 
// Shutdown from login page.
//

public class logout_screen_shutdown_successful extends TestSuiteBase {
	...
	// Start the test.
	@Test
	public void test_logout_screen_shutdown_successful() {
		...
		try {
			...
			captureScreenShot();
			 ...
			 // Check the shutdown button status
			if(!SelUtil.verifyButtonStatus("LoginPage_ShutdownButton","enabled")) {
				throw new TestException("Test failed");
			}
			captureScreenShot();
			...
			// Click on yes,shutdown button
			if(!SelUtil.clickByXpath("LoginPage_ShutdownYesShutdownButton")){
				throw new TestException("Test failed");
			}
			captureScreenShot();
			...
			// Check whether login page is not present
			if(!SelUtil.waitForNoElementByXpath("LoginPage_Heading")){
				printError("Shutdown is not successfull");
				throw new TestException("Test failed");
			}
			captureScreenShot();
