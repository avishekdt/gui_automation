...
public class dn_installation_option_rewrite_both_lin extends TestSuiteBase {	
	...
	@Test
	public void test_dn_installation_option_rewrite_both() {
		...
		try {
			...
			if(!killRemoteHpsum(remoteHostDesc_l1, remoteHostIp_l1, remoteHostUserName1_l1, remoteHostPassword1_l1)) {
				printError("Failed to kill hpsum service in dn_installation_option_rewrite_both");
			}
			...
			if(!Nodes.deployNodeAdvMode_installation_Options(remoteHostIp_l1, remoteHostType_l1,"Rewrite Both")) {
				throw new TestException("Test Failed");
			}
