...
public class dn_installation_option_upgrade_firmware_lin extends TestSuiteBase {	
	...
	@Test
	public void test_dn_installation_option_upgrade_firmware() {
		...
		try {
			...
			if(!killRemoteHpsum(remoteHostDesc_l1, remoteHostIp_l1, remoteHostUserName1_l1, remoteHostPassword1_l1)) {
				printError("Failed to kill hpsum service in dn_installation_option_upgrade_firmware");
			}
			...
			if(!Nodes.deployNodeAdvMode_installation_Options(remoteHostIp_l1, remoteHostType_l1,"Upgrade Firmware")) {
				throw new TestException("Test Failed");
			}
