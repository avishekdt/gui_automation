...
public class dn_installation_option_upgrade_software_lin extends TestSuiteBase {	
	...
	@Test
	public void test_dn_installation_option_upgrade_software() {
		...
		try {
			...
			/*Before running this test case, do the deployment using gen9snap3 SPP so that we can select 
			 * upgrade software option and check the components weather selected correctly or not.
			   Location_toSelect -  provide the spp path in the config.properties file and
			    give the value for this variable.
			*/
			...
			if(!killRemoteHpsum(remoteHostDesc_l1, remoteHostIp_l1, remoteHostUserName1_l1, remoteHostPassword1_l1)) {
				printError("Failed to kill hpsum service in dn_installation_option_upgrade_software");
			}
			...
			if(!Nodes.deployNodeAdvMode_installation_Options(remoteHostIp_l1, remoteHostType_l1,"Upgrade Software")) {
				throw new TestException("Test Failed");
			}
