...
public class dn_advanced_mode_upgrade_both1 extends TestSuiteBase {	
	...
	@Test
	public void test_dn_advanced_mode_upgrade_both1() {
		...
		try {
			...
			if(!killRemoteHpsum(remoteHostDesc_w1, remoteHostIp_w1, remoteHostUserName1_w1, remoteHostPassword1_w1)) {
				printError("Failed to kill hpsum service in dn_advanced_mode_upgrade_both1");
			}
			...
			if(!Nodes.deployNodeAdvMode_installation_Options(remoteHostIp_w1, remoteHostType_w1,"Upgrade Firmware and Software")) {
				throw new TestException("Test Failed");
			}
