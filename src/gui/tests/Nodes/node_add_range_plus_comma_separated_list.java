...
public class node_add_range_plus_comma_separated_list extends TestSuiteBase {	
	...
	public void test_template_test_1() {
		...
		try {
			...
			String remoteHostIp_w1_range = CONFIG.getProperty("remoteHostIp_w1_range");
			String remoteHostIpList_w1 		= remoteHostIp_w1+','+ remoteHostIp_w1_range;
			...
			//
			// Verification:
			// Check the nodes which were added are present in nodes list in nodes page.
			printLogs("Verifying the nodes added are present in nodes list in nodes page.");
			List<String> WinNodeList = Arrays.asList(remoteHostIp_w1_range.split("-"));
			...
			if(!Nodes.findNodeOnLeftPane(remoteHostIp_w1)){
				throw new TestException("Test Failed");
			}
