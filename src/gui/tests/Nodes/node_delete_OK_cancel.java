...
public class node_delete_OK_cancel extends TestSuiteBase {	
	...
	public void test_template_test_1() {
		...
		try {
			...
			if(!Nodes.deleteNode(remoteHostIp_w1)) {
			throw new TestException("Test Failed");
			}
			//
			// Verification:
			if(Nodes.findNodeOnLeftPane(remoteHostIp_w1)==false)
				{
				captureScreenShot();
				printLogs(remoteHostIp_w1+" is not present in Nodes list in Nodes page, Node is deleted successfully.");
				}
			else 
				throw new TestException("Test Failed");
