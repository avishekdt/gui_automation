 ...
 * Author: Praveen AC
 * Date: 05/26/2016
 * Test case Description: Add a VC where VC and OA having different credentials. Select "use credentials
  						 above" and and add the VC. Do inventory. Inventory should not proceed and user 
  						 should get proper error message nad there should be option to edit OA credentials
 * Prerequisites: NA
 * Steps:
 * 1. Launch HPSUM.
...
public class enc_vc_add_with_diff_oa_cred extends TestSuiteBase{
...
public void test_enc_vc_add_secondary_vc_error() {		
		...
		try {
			...
			if(!Nodes.addNodeVc(remoteHostIp_vc1, remoteHostDesc_vc1, false, false,remoteHostUserName1_vc1, 
					remoteHostPassword1_vc1,remoteHostIp_oa1,remoteHostUserName1_vc1,remoteHostPassword1_vc1)) {
			throw new TestException("Test Failed");
			}
