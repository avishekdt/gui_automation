 ...
 * Author: Praveen AC
 * Date: 05/27/2016
 * Test case Description: Try to add a VC with invalid OA credentials from the nodes page. 
  				Proper error message should be displayed and the option to edit the oa credentials 
  				should be provided. Edit the node and enter correct oa credentials . 
  				Invenory and deployment should be successful
 * Prerequisites: NA
 * Steps:
 * 1. Launch HPSUM.
 * 2. Do Baseline Inventory.
 * 3. Navigate to context screen and add VC by providing the wrong credentials for OA.
 * 4. Perform Inventory and then enter the correct credentials.
...
public class enc_vc_add_with_wrong_oa_cred extends TestSuiteBase{
	...
	@Test
	public void test_enc_vc_add_with_wrong_oa_cred() {		
		...
		try {
			...
			String remoteHostPassword1_oa_wrong = "1234abc";
			...
			//3. Adding a node.
			if(!Nodes.addNodeVc(remoteHostIp_vc1, remoteHostDesc_vc1, false, false,remoteHostUserName1_vc1, 
					remoteHostPassword1_vc1,remoteHostIp_oa1,remoteHostUserName1_oa1,remoteHostPassword1_oa_wrong)) {
			throw new TestException("Test Failed");
			}
			
			Nodes.inv_error = true;
			//4. Performing the inventory
			if(!Nodes.performNodeInventory()){
				throw new TestException("Test Failed");
			}
			Nodes.inv_error = false;
			
			//5. Editing the Associated Node Details
			if(!Nodes.editNodeCredentials(remoteHostIp_vc1,remoteHostUserName1_oa1,remoteHostPassword1_oa1)){
				throw new TestException("Test Failed");
			}
			//Waiting for the inventory to complete
